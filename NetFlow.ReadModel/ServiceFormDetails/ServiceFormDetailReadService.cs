...
{
    public class ServiceFormDetailReadService
    {
        private readonly ReadModelOptions _opt;
        public ServiceFormDetailReadService(ReadModelOptions opt) => _opt = opt;
        ...
        public async Task<PagedResult> ListAsync(int userId, int serviceFormId, PagedRequest pagedRequest)
        {
            ...
            string countSql = $@"
                SELECT COUNT(1) FROM dbo.VW_ServiceFormDetail WITH (NOLOCK)
                {whereSql}
            ";

            string dataSql = $@"
                SELECT * FROM dbo.VW_ServiceFormDetail WITH (NOLOCK)
                {whereSql}
                {orderBy}
            ...
            if (pagedRequest.IsCountQuery != null && pagedRequest.IsCountQuery.HasValue)
            {
                ...
                {
                    Data = Array.Empty<ServiceFormDetailDto>(),
            ...
            var data = cn.Query<ServiceFormDetailDto>(dataSql, parameters).ToList();
        ...
        public async Task<ServiceFormDetailDto?> GetAsync(int id)
        {
            using var cn = new SqlConnection(_opt.ConnectionString);

            var sql = "SELECT TOP 1 * FROM  dbo.VW_ServiceFormDetail WITH (NOLOCK) WHERE Id=@Id";
            return await cn.QueryFirstOrDefaultAsync<ServiceFormDetailDto>(sql, new { Id = id });
        }
    }
}
