...
namespace NetFlow.ReadModel.ServiceDetails
{
    public class ServiceDetailReadService
    {
        private readonly ReadModelOptions _opt;
        public ServiceDetailReadService(ReadModelOptions opt) => _opt = opt;
        ...
        public async Task<PagedResult> ListAsync(int userId, int serviceFormId, PagedRequest pagedRequest)
        {
            ...
            string countSql = $@"
                SELECT COUNT(1) FROM dbo.VW_ServiceDetail WITH (NOLOCK)
                {whereSql}
            ";

            string dataSql = $@"
                SELECT * FROM dbo.VW_ServiceDetail WITH (NOLOCK)
                {whereSql}
                {orderBy}
            ...
            if (pagedRequest.IsCountQuery != null && pagedRequest.IsCountQuery.HasValue)
            {
                ...
                {
                    Data = Array.Empty<ServiceDetailDto>(),
            ...
            if (!string.IsNullOrWhiteSpace(pagedRequest.TotalSummary))
            {
                ...
                string sql = $@"
                    ...
                    FROM dbo.VW_ServiceDetail WITH (NOLOCK)
                    {whereSql};
                ";
            ...
            var data = cn.Query<ServiceDetailDto>(dataSql, parameters).ToList();
        ...
        public async Task<ServiceDetailDto?> GetAsync(int id)
        {
            using var cn = new SqlConnection(_opt.ConnectionString);

            var sql = "SELECT TOP 1 * FROM  dbo.VW_ServiceDetail WITH (NOLOCK) WHERE Id=@Id";
            return await cn.QueryFirstOrDefaultAsync<ServiceDetailDto>(sql, new { Id = id });
        }
    }
}
