...
namespace NetFlow.ReadModel.ServiceTechnicians
{
    public class ServiceTechnicianReadService
    {
        private readonly ReadModelOptions _opt;
        public ServiceTechnicianReadService(ReadModelOptions opt) => _opt = opt;
        ...
        public async Task<PagedResult> ListAsync(int userId, int serviceFormId, PagedRequest pagedRequest)
        {
            ...
            string countSql = $@"
                SELECT COUNT(1) FROM dbo.VW_ServiceTechnician WITH (NOLOCK)
                {whereSql}
            ";

            string dataSql = $@"
                SELECT * FROM dbo.VW_ServiceTechnician WITH (NOLOCK)
                {whereSql}
                {orderBy}
            ...
            if (pagedRequest.IsCountQuery != null && pagedRequest.IsCountQuery.HasValue)
            {
                ...
                {
                    Data = Array.Empty<ServiceTechnicianDto>(),
            ...
            if (!string.IsNullOrWhiteSpace(pagedRequest.TotalSummary))
            {
                ...
                string sql = $@"
                    ...
                    FROM dbo.VW_ServiceTechnician WITH (NOLOCK)
                    {whereSql};
                ";
            ...
            var data = cn.Query<ServiceTechnicianDto>(dataSql, parameters).ToList();
        ...
        public async Task<ServiceTechnicianDto?> GetAsync(int id)
        {
            using var cn = new SqlConnection(_opt.ConnectionString);

            var sql = "SELECT TOP 1 * FROM dbo.VW_ServiceTechnician WITH (NOLOCK) WHERE Id=@Id";
            return await cn.QueryFirstOrDefaultAsync<ServiceTechnicianDto>(sql, new { Id = id });
        }
    }
}
