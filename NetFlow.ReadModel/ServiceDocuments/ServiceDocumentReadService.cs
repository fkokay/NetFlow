...
namespace NetFlow.ReadModel.ServiceDocuments
{
    public class ServiceDocumentReadService
    {
        private readonly ReadModelOptions _opt;

        public ServiceDocumentReadService(ReadModelOptions opt) => _opt = opt;
        ...
        public async Task<PagedResult> ListAsync(int userId, int serviceFormId, PagedRequest pagedRequest)
        {
            ...
            string countSql = $@"
                SELECT COUNT(1) FROM dbo.VW_ServiceDocument WITH (NOLOCK)
                {whereSql}
            ";

            string dataSql = $@"
                SELECT * FROM dbo.VW_ServiceDocument WITH (NOLOCK)
                {whereSql}
                {orderBy}
            ...
            if (pagedRequest.IsCountQuery != null && pagedRequest.IsCountQuery.HasValue)
            {
                ...
                {
                    Data = Array.Empty<ServiceDocumentDto>(),
            ...
            if (!string.IsNullOrWhiteSpace(pagedRequest.TotalSummary))
            {
                ...
                string sql = $@"
                    ...
                    FROM dbo.VW_ServiceDocument WITH (NOLOCK)
                    {whereSql};
                ";
            ...
            var data = cn.Query<ServiceDocumentDto>(dataSql, parameters).ToList();
        ...
        public async Task<ServiceDocumentDto?> GetAsync(int id)
        {
            using var cn = new SqlConnection(_opt.ConnectionString);
            var sql = "SELECT TOP 1 * FROM  dbo.VW_ServiceDocument WITH (NOLOCK) WHERE Id=@Id";
            return await cn.QueryFirstOrDefaultAsync<ServiceDocumentDto>(sql, new { Id = id });
        }
    }
}
