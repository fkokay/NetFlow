...
{
    public class ServiceReplacedPartReadService
    {
        private readonly ReadModelOptions _opt;
        public ServiceReplacedPartReadService(ReadModelOptions opt) => _opt = opt;
        ...
        public async Task<PagedResult> ListAsync(int userId, int serviceFormId, PagedRequest pagedRequest)
        {
            ...
            string countSql = $@"
                SELECT COUNT(1) FROM dbo.VW_ServiceReplacedPart WITH (NOLOCK)
                {whereSql}
            ";

            string dataSql = $@"
                SELECT * FROM dbo.VW_ServiceReplacedPart WITH (NOLOCK)
                {whereSql}
                {orderBy}
            ...
            if (pagedRequest.IsCountQuery != null && pagedRequest.IsCountQuery.HasValue)
            {
                ...
                {
                    Data = Array.Empty<ServiceReplacedPartDto>(),
            ...
            if (!string.IsNullOrWhiteSpace(pagedRequest.TotalSummary))
            {
                ...
                string sql = $@"
                    ...
                    FROM dbo.VW_ServiceReplacedPart WITH (NOLOCK)
                    {whereSql};
                ";
            ...
            var data = cn.Query<ServiceReplacedPartDto>(dataSql, parameters).ToList();
        ...
        public async Task<ServiceReplacedPartDto?> GetAsync(int id)
        {
            using var cn = new SqlConnection(_opt.ConnectionString);

            var sql = "SELECT TOP 1 * FROM dbo.VW_ServiceReplacedPart WITH (NOLOCK) WHERE Id=@Id";
            return await cn.QueryFirstOrDefaultAsync<ServiceReplacedPartDto>(sql, new { Id = id });
        }
