...
{
    public class ServiceFormTechnicianReadService
    {
        private readonly ReadModelOptions _opt;
        public ServiceFormTechnicianReadService(ReadModelOptions opt) => _opt = opt;
        ...
        public async Task<PagedResult> ListAsync(int userId, int serviceFormId, PagedRequest pagedRequest)
        {
            ...
            string countSql = $@"
                SELECT COUNT(1) FROM dbo.VW_ServiceFormTechnician WITH (NOLOCK)
                {whereSql}
            ";

            string dataSql = $@"
                SELECT * FROM dbo.VW_ServiceFormTechnician WITH (NOLOCK)
                {whereSql}
                {orderBy}
            ...
            if (pagedRequest.IsCountQuery != null && pagedRequest.IsCountQuery.HasValue)
            {
                ...
                {
                    Data = Array.Empty<ServiceFormTechnicianDto>(),
            ...
            if (!string.IsNullOrWhiteSpace(pagedRequest.TotalSummary))
            {
                ...
                string sql = $@"
                    ...
                    FROM dbo.VW_ServiceFormTechnician WITH (NOLOCK)
                    {whereSql};
                ";
            ...
            var data = cn.Query<ServiceFormTechnicianDto>(dataSql, parameters).ToList();
        ...
        public async Task<ServiceFormTechnicianDto?> GetAsync(int id)
        {
            using var cn = new SqlConnection(_opt.ConnectionString);

            var sql = "SELECT TOP 1 * FROM dbo.VW_ServiceFormTechnician WITH (NOLOCK) WHERE Id=@Id";
            return await cn.QueryFirstOrDefaultAsync<ServiceFormTechnicianDto>(sql, new { Id = id });
        }
    }
}
